...
def guess_linear():
    ...
    num_guesses = 1
    guess = LOWER
    while not is_correct(guess):    # ask user if guess is correct
        guess = guess + 1 # linear guess
        num_guesses = num_guesses + 1
...
def guess_binary():
    ...
    guess = (lower + upper) // 2
    while not is_correct(guess):    # ask user if guess is correct
        if is_too_high(guess):  # ask user if guess is too high
            upper = guess
            guess = (lower + upper) // 2
        else:
            if guess == upper - 1:  # edge case, lower = guess = upper - 1
                lower = upper
            else:
                lower = guess
            guess = (lower + upper) // 2
